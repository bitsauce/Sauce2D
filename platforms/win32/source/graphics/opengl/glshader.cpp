...
GLshader::GLshader(const string &vertFilePath, const string &fragFilePath)
{
	...
	// Read vertex source
	fileReader = XFileSystem::CreateFileReader(util::getAbsoluteFilePath(vertFilePath));
	string vertSource = fileReader->readAll();
	fileReader->close();
	delete fileReader;
	
	// Read fragment source
	fileReader = XFileSystem::CreateFileReader(util::getAbsoluteFilePath(fragFilePath));
...
void GLshader::setSampler2D(const string &name, XTexture *texture)
{

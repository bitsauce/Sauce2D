...
void xdDebug::sendPacket(xdPacketType type, const char *data)
{
...
void xdDebug::recvPacket(char **data)
{
	// Make sure we have a connection
	if(!m_connected)
		return;
...
void xdDebug::lineCallback(void *ctxptr)
{
	// Cast ctx
...
bool xdDebug::isBreakpoint(void *ctxptr)
{
	// Cast ctx
	asIScriptContext *ctx = static_cast<asIScriptContext*>(ctxptr);
...
void xdDebug::takeCommands(void *ctxptr)
{
	// Cast ctx
	asIScriptContext *ctx = static_cast<asIScriptContext*>(ctxptr);

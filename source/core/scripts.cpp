...
{
	asIScriptObject *scriptObject = (asIScriptObject*)obj;
	return scriptObject->Release();
}

int releaseScriptFunc(void *func)
{
	asIScriptFunction *scriptFunc = (asIScriptFunction*)func;
	return scriptFunc->Release();
}

asIScriptContext *currentCtx = 0;
vector<ScriptArgument> currentArgs;
int startScriptFuncCall(void *func)
{
	if(currentCtx != 0) return -1;
	currentCtx = 0;//xdEngine::GetScripts()->createContext();
	currentArgs.reserve(((asIScriptFunction*)func)->GetParamCount());
	int r = currentCtx->Prepare((asIScriptFunction*)func);
	if(r < 0)
	{
		ERR("startScriptFuncCall failed with the error code %i", r);
		assert(false);
	}
	return r;
}
...
int endScriptFuncCall()
{
	...
	if(r < 0)
	{
		ERR("endScriptFuncCall failed with the error code %i", r);
		assert(false);
	}

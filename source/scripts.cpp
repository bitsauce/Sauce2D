...
int registerGlobalFunction(const char *funcDef, const asSFuncPtr &funcPointer)
{
	...
	if(r < 0) {
		iosystem::error("registerGlobalFunction failed with the error code %i, with funcDef '%s'", r, funcDef);
		assert(false);
	}
	return r;
}
...
int registerFunctionDef(const char *decl)
{
	...
	if(r < 0) {
		iosystem::error("registerFunctionDef failed with the error code %i, with decl '%s'", r, decl);
		assert(false);
	}
	return r;
}
...
int startScriptFuncCall(void *func)
{
	...
	currentArgs.reserve(((ScriptFunction*)func)->GetParamCount());
	int r = currentCtx->Prepare((ScriptFunction*)func);
	if(r < 0) {
		iosystem::error("startScriptFuncCall failed with the error code %i", r);
		assert(false);
	}
	return r;
}
...
{
	if(currentCtx == 0) return -1;
	currentArgs.push_back(ScriptArgument());
...
int endScriptFuncCall()
{
	...
	for(uint i = 0; i < currentArgs.size(); i++)
	{
		...
		else if(arg->typeId & asTYPEID_OBJHANDLE) // Handle type
		{
			...
			currentCtx->SetArgQWord(i, *(asQWORD*)arg->value);
	}

	// Execute script func
	int r = currentCtx->Execute();
	if(r < 0) {
		iosystem::error("endScriptFuncCall failed with the error code %i", r);
		assert(false);
	}
...
int registerEnum(const char* name)
{
	...
	if(r < 0) {
		iosystem::error("registerEnum failed with the error code %i", r);
		assert(false);
	}
	return r;
}
...
int registerEnumValue(const char *enumname, const char *valuename, int value)
{
	...
	if(r < 0) {
		iosystem::error("registerEnumValue failed with the error code %i", r);
		assert(false);
	}
	return r;
}
...
{
	return gameEngine->scripts->module()->GetFunctionByDecl(decl);
}
...
ScriptContext *ScriptManager::createScriptContext() const
{
	...
	if(gameEngine->isEnabled(X2D_Debug))
		ctx->SetLineCallback(asMETHOD(X2DDebug, lineCallback), gameEngine->debug, asCALL_THISCALL);
	return ctx;
}
...
void ScriptManager::createObject(void *object, int typeId, const string decl) const
{
	...
	if(!m_createObjectType) {
		iosystem::error("ScriptManager::createObject() No object was defined!");
		return;
	}
	...
	if(func->GetParamCount() != m_createArgs.size()) {
		iosystem::error("ScriptManager::createObject() Invalid number of arguments (got %i, expected %i)!", m_createArgs.size(), func->GetParamCount());
		return;
	}
...
void ScriptArgument::clear()
{
	...
	if(valid) {
		if(typeId == asTYPEID_VOID || typeId & asTYPEID_MASK_OBJECT)
...
void asMessageCallback(const asSMessageInfo *msg, void*)
{
	...
	if(gameEngine->isEnabled(X2D_Debug))
		gameEngine->debug->sendPacket(X2DDebug::CompilePacket, str.c_str());
	else
		messageCallback += str;
}
...
int asCompileModule(string name)
{
	...
	while(error)
	{
		...
		if(r < 0)
		{
			...
			if(gameEngine->isEnabled(X2D_Debug))
				gameEngine->debug->sendMessage("Unable to load main.as. Make sure the file and other included files exist.", X2D_ErrorMsg);
			else

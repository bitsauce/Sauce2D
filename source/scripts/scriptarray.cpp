...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "T &at(uint)", asMETHODPR(CScriptArray, At, (asUINT), void*), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("array<T>", "void clear()", asMETHOD(CScriptArray, Clear), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sort(SortFunc @func)", asMETHODPR(CScriptArray, StdSort, (asIScriptFunction*), void), asCALL_THISCALL); assert( r >= 0 );
...
void CScriptArray::StdSort(asIScriptFunction *func)
{
	int start = 0;

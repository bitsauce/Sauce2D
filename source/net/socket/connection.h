...
namespace net
{
	...
	public:
		...
		virtual bool SendPacket( const unsigned char data[], int size )
		{
			...
			if ( address.GetAddress() == 0 )
				return false;
			unsigned char *packet = new unsigned char[size + NET_PACKET_HEADER];
			packet[0] = (unsigned char) ( protocolId >> 24 );
			packet[1] = (unsigned char) ( ( protocolId >> 16 ) & 0xFF );
			packet[2] = (unsigned char) ( ( protocolId >> 8 ) & 0xFF );
			packet[3] = (unsigned char) ( ( protocolId ) & 0xFF );
			memcpy( &packet[4], data, size );
			return socket.Send(address, packet, size + NET_PACKET_HEADER);
		}
		...
		virtual int ReceivePacket( unsigned char data[], int size )
		{
			...
			if ( sender == address )
			{
				...
				memcpy( data, &packet[4], bytes_read - NET_PACKET_HEADER );
				return bytes_read - NET_PACKET_HEADER;
			}
			return 0;
		}
	...
	public:
		...
		bool SendPacket( const unsigned char data[], int size )
		{
			...
			packet.WriteBytes(data, size);
			if ( !Connection::SendPacket( packet.GetPacketInfo(), size + header ) )
				return false;
			reliabilitySystem.PacketSent( size );
			return true;
		}	
		...
		int ReceivePacket( unsigned char data[], int size )
		{
			...
			if ( size <= header )
				return false;
			NetPacket packet;
			int received_bytes = Connection::ReceivePacket( packet.GetPacketInfo(), size + header );
			if ( received_bytes == 0 )
				return false;
			...
			packet.WriteBytes(data, received_bytes - header);
	...
	protected:		
		...
		void WriteHeader( NetPacket* packet, unsigned int sequence, unsigned int ack, unsigned int ack_bits )
		{
			packet->WriteInteger( sequence );
			packet->WriteInteger( ack );
			packet->WriteInteger( ack_bits );
		}

		void ReadHeader( NetPacket* packet, unsigned int & sequence, unsigned int & ack, unsigned int & ack_bits )
		{
			packet->ReadInteger( sequence );
			packet->ReadInteger( ack );
			packet->ReadInteger( ack_bits );
		}

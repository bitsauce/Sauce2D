...
namespace net
{
	...
	public:
		...
		void Update(float deltaTime)
		{
			...
			// Send update/keep alive packets to nodes
	...
	protected:
		...
		void ProcessPacket( const Address & sender, unsigned char data[], int size )
		{
			...
			unsigned int protocolIdPacket = ( unsigned(data[0]) << 24 ) | ( unsigned(data[1]) << 16 ) |
			                                    ( unsigned(data[2]) << 8 )  | unsigned(data[3]);
			if(protocolIdPacket != protocolId)
				return;
		...
		void SendPackets( float deltaTime )
		{
			...
			while ( sendAccumulator > sendRate )
			{
				for ( unsigned int i = 0; i < nodes.size(); ++i )
				{
					...
					{
						// Node is negotiating join: Send "Connection Accepted" packet
						int packetsize = 7;
						unsigned char *packet = new unsigned char[packetsize];
						packet[0] = (unsigned char) ( ( protocolId >> 24 ) & 0xFF );
						packet[1] = (unsigned char) ( ( protocolId >> 16 ) & 0xFF );
						packet[2] = (unsigned char) ( ( protocolId >> 8 ) & 0xFF );
						packet[3] = (unsigned char) (unsigned char) ( ( protocolId ) & 0xFF );
						packet[4] = (unsigned char) ( 0 );
						packet[5] = (unsigned char) ( i );
						packet[6] = (unsigned char) (char)nodes.size();
					...
					else if ( nodes[i].mode == NodeState::Connected )
					{
						...
						int packetsize = 5 + 6 * nodes.size();
						unsigned char* packet = new unsigned char[packetsize];
						packet[0] = (unsigned char) ( ( protocolId >> 24 ) & 0xFF );
						packet[1] = (unsigned char) ( ( protocolId >> 16 ) & 0xFF );
						packet[2] = (unsigned char) ( ( protocolId >> 8 ) & 0xFF );
						packet[3] = (unsigned char) ( ( protocolId ) & 0xFF );
						packet[4] = (unsigned char) 1;
						packet[5] = (unsigned char) i;
		...
		void CheckForTimeouts(float deltaTime)
		{
			...
			for(unsigned int i = 0; i < nodes.size(); ++i)
			{
				...
				if(nodes[i].mode != NodeState::Disconnected)
				{
					// Disconnected mode, apply to accumulator
					nodes[i].timeoutAccumulator += deltaTime;

					// Timeout?
					if(nodes[i].timeoutAccumulator > timeout)
					{
						// Find node and erase it from the node mesh
	...
	protected:
		...
		void ReceivePackets()
		{
			...
			{
				Address sender;
				unsigned char data[NET_PACKET_MAX];
				int size = socket.Receive( sender, data, sizeof(data) );
				if ( !size )
					break;
				ProcessPacket( sender, data, size );
		...
		void SendPackets( float deltaTime )
		{
			...
			while ( sendAccumulator > sendRate )
			{
				...
				{
					// Node is joined: Send "Keep Alive" packet
					unsigned char packet[5];
					packet[0] = (unsigned char) ( ( protocolId >> 24 ) & 0xFF );
					packet[1] = (unsigned char) ( ( protocolId >> 16 ) & 0xFF );
					packet[2] = (unsigned char) ( ( protocolId >> 8 ) & 0xFF );

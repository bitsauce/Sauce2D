...
ALuint loadSound(const string& fileName)
{
	...
	if(!f)
	{
		iosystem::error("Could not read wav file '%s'", filePath);
	...
	{
		iosystem::error("Wav magic failed '%s'", filePath);
		return WAV_LOAD_ERROR;
	}

	if(std::string(magic) != "RIFF")
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file (no RIFF magic) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	if(fread(magic, 4, 1, f) != 1)
	{
		iosystem::error("More wav magic failed '%s'", filePath);
		return WAV_LOAD_ERROR;
	}

	if(string(magic) != "WAVE")
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file (no WAVE magic) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	if(fread(magic,4,1,f) != 1)
	{
		iosystem::error("Failed to read subchunk '%s'", filePath);
		return WAV_LOAD_ERROR;
	}

	if(string(magic) != "fmt ")
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file (no 'fmt' subchunk) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	{
		iosystem::error("Failed to read subchunk size '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	if(!(subChunk1Size >= 16))
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file ('fmt ' chunk too small, truncated file?) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	{
		iosystem::error("Failed to read wav file '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	if(audioFormat != 1)
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file (audio format is not PCM) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	{
		iosystem::error("Wrong wav file format. This file is not a .wav file (no data subchunk) '%s'", filePath);
		return WAV_LOAD_ERROR;
	}
	...
	{
		iosystem::error("LoadWav: freq and frequency different sizes");
		return WAV_LOAD_ERROR;
	}
	...
	{
		iosystem::error("LoadWav: Could not generate buffer");
		return WAV_LOAD_ERROR;
	}
	...
	if(alGetError() != AL_NO_ERROR)
	{
		iosystem::error("LoadWav: Could not load buffer data");
		return WAV_LOAD_ERROR;
	}
...
void OpenALManager::init()
{
	...
	//iosystem::print("** Initializing OpenAL: Using default device '%s' **", defaultDevice);

	// Open device
	ALCdevice *device;
	if((device = alcOpenDevice(defaultDevice)) == NULL) {
		// Failed
		//iosystem::error("Error: failed to open sound device\n");
		return;
	}

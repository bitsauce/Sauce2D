...
//--------------------------------------------------------------------------
// XScriptDictionary implementation

XScriptDictionary *XScriptDictionary::Create(asIScriptEngine *engine)
{
	// Use the custom memory routine from AngelScript to allow application to better control how much memory is used
	XScriptDictionary *obj = (XScriptDictionary*)asAllocMem(sizeof(XScriptDictionary));
	new(obj) XScriptDictionary(engine);
	return obj;
}

XScriptDictionary *XScriptDictionary::Create(asBYTE *buffer)
{
	// Use the custom memory routine from AngelScript to allow application to better control how much memory is used
	XScriptDictionary *obj = (XScriptDictionary*)asAllocMem(sizeof(XScriptDictionary));
	new(obj) XScriptDictionary(buffer);
	return obj;
}

XScriptDictionary::XScriptDictionary(asIScriptEngine *engine)
{
...
XScriptDictionary::XScriptDictionary(asBYTE *buffer)
{
	// We start with one reference
	refCount = 1;
	gcFlag = false;
...
XScriptDictionary::~XScriptDictionary()
{
	// Delete all keys and values
	DeleteAll();
}

void XScriptDictionary::AddRef() const
{
...
void XScriptDictionary::Release() const
{
	// We need to clear the GC flag
	gcFlag = false;
	if( asAtomicDec(refCount) == 0 )
	{
		this->~XScriptDictionary();
		asFreeMem(const_cast<XScriptDictionary*>(this));
	}
}

int XScriptDictionary::GetRefCount()
{
	return refCount;
}

void XScriptDictionary::SetGCFlag()
{
	gcFlag = true;
}

bool XScriptDictionary::GetGCFlag()
{
	return gcFlag;
}

void XScriptDictionary::EnumReferences(asIScriptEngine *engine)
{
...
void XScriptDictionary::ReleaseAllReferences(asIScriptEngine * /*engine*/)
{
...
XScriptDictionary &XScriptDictionary::operator =(const XScriptDictionary &other)
{
...
CScriptDictValue *XScriptDictionary::operator[](const string &key)
{
...
const CScriptDictValue *XScriptDictionary::operator[](const string &key) const
{
...
void XScriptDictionary::Set(const string &key, void *value, int typeId)
{
	map<string, CScriptDictValue>::iterator it;
	it = dict.find(key);
	if( it == dict.end() )
...
// different type.
void XScriptDictionary::Set(const string &key, const asINT64 &value)
{
...
void XScriptDictionary::Set(const string &key, const double &value)
{
	Set(key, const_cast<double*>(&value), asTYPEID_DOUBLE);
}

// Returns true if the value was successfully retrieved
bool XScriptDictionary::Get(const string &key, void *value, int typeId) const
{
	map<string, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
int XScriptDictionary::GetTypeId(const string &key) const
{
	map<string, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
bool XScriptDictionary::Get(const string &key, asINT64 &value) const
{
	return Get(key, &value, asTYPEID_INT64);
}

bool XScriptDictionary::Get(const string &key, double &value) const
{
	return Get(key, &value, asTYPEID_DOUBLE);
}

bool XScriptDictionary::Exists(const string &key) const
{
	map<string, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
bool XScriptDictionary::IsEmpty() const
{
	if( dict.size() == 0 )
		return true;
...
asUINT XScriptDictionary::GetSize() const
{
	return asUINT(dict.size());
}

void XScriptDictionary::Delete(const string &key)
{
	map<string, CScriptDictValue>::iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
void XScriptDictionary::DeleteAll()
{
	map<string, CScriptDictValue>::iterator it;
	for( it = dict.begin(); it != dict.end(); it++ )
		it->second.FreeValue(engine);
...
XScriptArray* XScriptDictionary::GetKeys() const
{
...
{
	*(XScriptDictionary**)gen->GetAddressOfReturnLocation() = XScriptDictionary::Create(gen->GetEngine());
}

void ScriptDictionaryListFactory_Generic(asIScriptGeneric *gen)
{
	asBYTE *buffer = (asBYTE*)gen->GetArgAddress(0);
	*(XScriptDictionary**)gen->GetAddressOfReturnLocation() = XScriptDictionary::Create(buffer);
}

void ScriptDictionaryAddRef_Generic(asIScriptGeneric *gen)
{
	XScriptDictionary *dict = (XScriptDictionary*)gen->GetObject();
	dict->AddRef();
}
...
{
	XScriptDictionary *dict = (XScriptDictionary*)gen->GetObject();
	XScriptDictionary *other = *(XScriptDictionary**)gen->GetAddressOfArg(0);
	*dict = *other;
	*(XScriptDictionary**)gen->GetAddressOfReturnLocation() = dict;
}
...
{
	XScriptDictionary *self = (XScriptDictionary*)gen->GetObject();
...
static void XScriptDictionaryGetKeys_Generic(asIScriptGeneric *gen)
{
	XScriptDictionary *self = (XScriptDictionary*)gen->GetObject();
	*(XScriptArray**)gen->GetAddressOfReturnLocation() = self->GetKeys();
}

static void XScriptDictionary_opIndex_Generic(asIScriptGeneric *gen)
{
	XScriptDictionary *self = (XScriptDictionary*)gen->GetObject();
...
static void XScriptDictionary_opIndex_const_Generic(asIScriptGeneric *gen)
{
	const XScriptDictionary *self = (const XScriptDictionary*)gen->GetObject();
	std::string *key = (std::string*)gen->GetAddressOfArg(0);
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectType("dictionary", sizeof(XScriptDictionary), asOBJ_REF | asOBJ_GC); assert( r >= 0 );
	...
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asMETHOD(XScriptDictionary,AddRef), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASE, "void f()", asMETHOD(XScriptDictionary,Release), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "dictionary &opAssign(const dictionary &in)", asMETHODPR(XScriptDictionary, operator=, (const XScriptDictionary &), XScriptDictionary&), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const ?&in)", asMETHODPR(XScriptDictionary,Set,(const string&,void*,int),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const", asMETHODPR(XScriptDictionary,Get,(const string&,void*,int) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64&in)", asMETHODPR(XScriptDictionary,Set,(const string&,const asINT64&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64&out) const", asMETHODPR(XScriptDictionary,Get,(const string&,asINT64&) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const double&in)", asMETHODPR(XScriptDictionary,Set,(const string&,const double&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, double&out) const", asMETHODPR(XScriptDictionary,Get,(const string&,double&) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "bool exists(const string &in) const", asMETHOD(XScriptDictionary,Exists), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool isEmpty() const", asMETHOD(XScriptDictionary, IsEmpty), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "uint getSize() const", asMETHOD(XScriptDictionary, GetSize), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "void delete(const string &in)", asMETHOD(XScriptDictionary,Delete), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "void deleteAll()", asMETHOD(XScriptDictionary,DeleteAll), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "array<string> @getKeys() const", asMETHOD(XScriptDictionary,GetKeys), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)", asMETHODPR(XScriptDictionary, operator[], (const string &), CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "const dictionaryValue &opIndex(const string &in) const", asMETHODPR(XScriptDictionary, operator[], (const string &) const, const CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );

	// Register GC behaviours
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(XScriptDictionary,GetRefCount), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(XScriptDictionary,SetGCFlag), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(XScriptDictionary,GetGCFlag), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(XScriptDictionary,EnumReferences), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(XScriptDictionary,ReleaseAllReferences), asCALL_THISCALL); assert( r >= 0 );

#if AS_USE_STLNAMES == 1
	// Same as isEmpty
	r = engine->RegisterObjectMethod("dictionary", "bool empty() const", asMETHOD(XScriptDictionary, IsEmpty), asCALL_THISCALL); assert( r >= 0 );
	// Same as getSize
	r = engine->RegisterObjectMethod("dictionary", "uint size() const", asMETHOD(XScriptDictionary, GetSize), asCALL_THISCALL); assert( r >= 0 );
	// Same as delete
	r = engine->RegisterObjectMethod("dictionary", "void erase(const string &in)", asMETHOD(XScriptDictionary,Delete), asCALL_THISCALL); assert( r >= 0 );
	// Same as deleteAll
	r = engine->RegisterObjectMethod("dictionary", "void clear()", asMETHOD(XScriptDictionary,DeleteAll), asCALL_THISCALL); assert( r >= 0 );
#endif
}
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("dictionary", "array<string> @getKeys() const", asFUNCTION(XScriptDictionaryGetKeys_Generic), asCALL_GENERIC); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)", asFUNCTION(XScriptDictionary_opIndex_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "const dictionaryValue &opIndex(const string &in) const", asFUNCTION(XScriptDictionary_opIndex_const_Generic), asCALL_GENERIC); assert( r >= 0 );
...
XScriptDictionary::CIterator XScriptDictionary::begin() const
{
	return CIterator(*this, dict.begin());
}

XScriptDictionary::CIterator XScriptDictionary::end() const
{
	return CIterator(*this, dict.end());
}

XScriptDictionary::CIterator::CIterator(
		const XScriptDictionary &dict,
		std::map<std::string, CScriptDictValue>::const_iterator it)
	: m_it(it), m_dict(dict)
{}

void XScriptDictionary::CIterator::operator++() 
{ 
	++m_it; 
}

void XScriptDictionary::CIterator::operator++(int) 
{ 
	++m_it;
...
XScriptDictionary::CIterator &XScriptDictionary::CIterator::operator*()
{
	return *this;
}

bool XScriptDictionary::CIterator::operator==(const CIterator &other) const 
{ 
...
const std::string &XScriptDictionary::CIterator::GetKey() const 
{ 
	return m_it->first; 
}

int XScriptDictionary::CIterator::GetTypeId() const
{ 
	return m_it->second.m_typeId; 
}

bool XScriptDictionary::CIterator::GetValue(asINT64 &value) const
{ 
	return m_it->second.Get(m_dict.engine, &value, asTYPEID_INT64); 
}

bool XScriptDictionary::CIterator::GetValue(double &value) const
{ 
	return m_it->second.Get(m_dict.engine, &value, asTYPEID_DOUBLE); 
}

bool XScriptDictionary::CIterator::GetValue(void *value, int typeId) const
{ 

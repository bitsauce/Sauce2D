...
void XScriptArray::SetMemoryFunctions(asALLOCFUNC_t allocFunc, asFREEFUNC_t freeFunc)
{
	userAlloc = allocFunc;
	userFree = freeFunc;
}
...
XScriptArray* XScriptArray::Create(asIObjectType *ot, asUINT length)
{
	asIScriptContext *ctx = asGetActiveContext();

	// Allocate the memory
	void *mem = userAlloc(sizeof(XScriptArray));
	if( mem == 0 )
	{
		if( ctx )
	...
	XScriptArray *a = new(mem) XScriptArray(length, ot);
...
XScriptArray* XScriptArray::Create(asIObjectType *ot, void *initList)
{
	asIScriptContext *ctx = asGetActiveContext();
	...
	// Initialize the object
	XScriptArray *a = new(mem) XScriptArray(ot, initList);
...
XScriptArray* XScriptArray::Create(asIObjectType *ot, asUINT length, void *defVal)
{
	asIScriptContext *ctx = asGetActiveContext();
	...
	// Initialize the object
	XScriptArray *a = new(mem) XScriptArray(length, defVal, ot);
...
XScriptArray* XScriptArray::Create(asIObjectType *ot)
{
	return XScriptArray::Create(ot, asUINT(0));
}
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in)", asFUNCTIONPR(XScriptArray::Create, (asIObjectType*), XScriptArray*), asCALL_CDECL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint)", asFUNCTIONPR(XScriptArray::Create, (asIObjectType*, asUINT), XScriptArray*), asCALL_CDECL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint, const T &in)", asFUNCTIONPR(XScriptArray::Create, (asIObjectType*, asUINT, void *), XScriptArray*), asCALL_CDECL); assert( r >= 0 );

	// Register the factory that will be used for initialization lists
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_LIST_FACTORY, "array<T>@ f(int&in type, int&in list) {repeat T}", asFUNCTIONPR(XScriptArray::Create, (asIObjectType*, void*), XScriptArray*), asCALL_CDECL); assert( r >= 0 );

	// The memory management methods
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_ADDREF, "void f()", asMETHOD(XScriptArray,AddRef), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_RELEASE, "void f()", asMETHOD(XScriptArray,Release), asCALL_THISCALL); assert( r >= 0 );

	// The index operator returns the template subtype
	r = engine->RegisterObjectMethod("array<T>", "T &opIndex(uint)", asMETHODPR(XScriptArray, At, (asUINT), void*), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "const T &opIndex(uint) const", asMETHODPR(XScriptArray, At, (asUINT) const, const void*), asCALL_THISCALL); assert( r >= 0 );

	// The assignment operator
	r = engine->RegisterObjectMethod("array<T>", "array<T> &opAssign(const array<T>&in)", asMETHOD(XScriptArray, operator=), asCALL_THISCALL); assert( r >= 0 );

	// Other methods
	r = engine->RegisterFuncdef("bool SortFunc(int, int)"); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void insertAt(uint, const T&in)", asMETHOD(XScriptArray, InsertAt), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("array<T>", "void resize(uint)", asMETHODPR(XScriptArray, Resize, (asUINT), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void clear()", asMETHOD(XScriptArray, Clear), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sort(SortFunc @func)", asMETHODPR(XScriptArray, StdSort, (asIScriptFunction*), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sortAsc()", asMETHODPR(XScriptArray, SortAsc, (), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sortAsc(uint, uint)", asMETHODPR(XScriptArray, SortAsc, (asUINT, asUINT), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sortDesc()", asMETHODPR(XScriptArray, SortDesc, (), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void sortDesc(uint, uint)", asMETHODPR(XScriptArray, SortDesc, (asUINT, asUINT), void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void reverse()", asMETHOD(XScriptArray, Reverse), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "int find(const T&in) const", asMETHODPR(XScriptArray, Find, (void*) const, int), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "int find(uint, const T&in) const", asMETHODPR(XScriptArray, Find, (asUINT, void*) const, int), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "int findByRef(const T&in) const", asMETHODPR(XScriptArray, FindByRef, (void*) const, int), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "int findByRef(uint, const T&in) const", asMETHODPR(XScriptArray, FindByRef, (asUINT, void*) const, int), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "bool opEquals(const array<T>&in) const", asMETHOD(XScriptArray, operator==), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "bool isEmpty() const", asMETHOD(XScriptArray, IsEmpty), asCALL_THISCALL); assert( r >= 0 );

	// Register virtual properties
	r = engine->RegisterObjectMethod("array<T>", "uint get_size() const", asMETHOD(XScriptArray, GetSize), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void set_size(uint)", asMETHODPR(XScriptArray, Resize, (asUINT), void), asCALL_THISCALL); assert( r >= 0 );

	// Register GC behaviours in case the array needs to be garbage collected
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(XScriptArray, GetRefCount), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(XScriptArray, SetFlag), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(XScriptArray, GetFlag), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(XScriptArray, EnumReferences), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(XScriptArray, ReleaseAllHandles), asCALL_THISCALL); assert( r >= 0 );

#if AS_USE_STLNAMES == 1
	// Same as length
	r = engine->RegisterObjectMethod("array<T>", "uint size() const", asMETHOD(XScriptArray, GetSize), asCALL_THISCALL); assert( r >= 0 );
	// Same as isEmpty
	r = engine->RegisterObjectMethod("array<T>", "bool empty() const", asMETHOD(XScriptArray, IsEmpty), asCALL_THISCALL); assert( r >= 0 );
	// Same as insertLast
	r = engine->RegisterObjectMethod("array<T>", "void push_back(const T&in)", asMETHOD(XScriptArray, InsertLast), asCALL_THISCALL); assert( r >= 0 );
	// Same as removeLast
	r = engine->RegisterObjectMethod("array<T>", "void pop_back()", asMETHOD(XScriptArray, RemoveLast), asCALL_THISCALL); assert( r >= 0 );
	// Same as insertAt
	r = engine->RegisterObjectMethod("array<T>", "void insert(uint, const T&in)", asMETHOD(XScriptArray, InsertAt), asCALL_THISCALL); assert( r >= 0 );
	// Same as removeAt
	r = engine->RegisterObjectMethod("array<T>", "void erase(uint)", asMETHOD(XScriptArray, RemoveAt), asCALL_THISCALL); assert( r >= 0 );
#endif
}

XScriptArray &XScriptArray::operator=(const XScriptArray &other)
{
...
XScriptArray::XScriptArray(asIObjectType *ot, void *buf)
{
	refCount = 1;
	gcFlag = false;
	objType = ot;
...
XScriptArray::XScriptArray(asUINT length, asIObjectType *ot)
{
	refCount = 1;
	gcFlag = false;
	objType = ot;
...
XScriptArray::XScriptArray(const XScriptArray &other)
{
	refCount = 1;
	gcFlag = false;
...
XScriptArray::XScriptArray(asUINT length, void *defVal, asIObjectType *ot)
{
	refCount = 1;
	gcFlag = false;
	objType = ot;
...
void XScriptArray::SetValue(asUINT index, void *value)
{
...
asUINT XScriptArray::GetSize() const
{
	return buffer->numElements;
}

bool XScriptArray::IsEmpty() const
{
	return buffer->numElements == 0;
}

void XScriptArray::Reserve(asUINT maxElements)
{
...
void XScriptArray::Resize(asUINT numElements)
{
...
void XScriptArray::Clear()
{
	Resize(0);
}

// Internal
void XScriptArray::Resize(int delta, asUINT at)
{
	if( delta < 0 )
	{
...
// internal
bool XScriptArray::CheckMaxSize(asUINT numElements)
{
...
asIObjectType *XScriptArray::GetArrayObjectType() const
{
	return objType;
}

int XScriptArray::GetArrayTypeId() const
{
	return objType->GetTypeId();
}

int XScriptArray::GetElementTypeId() const
{
	return subTypeId;
}

string XScriptArray::GetElementTypeName() const
{
	return objType->GetSubType()->GetName();
}

void XScriptArray::InsertAt(asUINT index, void *value)
{
...
void XScriptArray::InsertLast(void *value)
{
	InsertAt(buffer->numElements, value);
}

void XScriptArray::RemoveAt(asUINT index)
{
	if( index >= buffer->numElements )
	{
		// If this is called from a script we raise a script exception
...
const void *XScriptArray::At(asUINT index) const
{
...
void *XScriptArray::At(asUINT index)
{
	return const_cast<void*>(const_cast<const XScriptArray *>(this)->At(index));
}


// internal
void XScriptArray::CreateBuffer(SArrayBuffer **buf, asUINT numElements)
{
...
// internal
void XScriptArray::DeleteBuffer(SArrayBuffer *buf)
{
...
// internal
void XScriptArray::Construct(SArrayBuffer *buf, asUINT start, asUINT end)
{
...
// internal
void XScriptArray::Destruct(SArrayBuffer *buf, asUINT start, asUINT end)
{
	if( subTypeId & asTYPEID_MASK_OBJECT )
	{
		asIScriptEngine *engine = objType->GetEngine();
...
// internal
bool XScriptArray::Less(const void *a, const void *b, bool asc, asIScriptContext *ctx, SArrayCache *cache)
{
	if( !asc )
	{
		// Swap items
...
bool XScriptArray::operator==(const XScriptArray &other) const
{
...
// internal
bool XScriptArray::Equals(const void *a, const void *b, asIScriptContext *ctx, SArrayCache *cache) const
{
	if( !(subTypeId & ~asTYPEID_MASK_SEQNBR) )
	{
		// Simple compare of values
...
int XScriptArray::FindByRef(void *ref) const
{
	return FindByRef(0, ref);
}

int XScriptArray::FindByRef(asUINT startAt, void *ref) const
{
...
int XScriptArray::Find(void *value) const
{
	return Find(0, value);
}

int XScriptArray::Find(asUINT startAt, void *value) const
{
...
void XScriptArray::Copy(void *dst, void *src)
{
...
void *XScriptArray::GetArrayItemPointer(int index)
{
...
void *XScriptArray::GetDataPointer(void *buffer)
{
	if ((subTypeId & asTYPEID_MASK_OBJECT) && !(subTypeId & asTYPEID_OBJHANDLE) )
	{
...
// Sort ascending
void XScriptArray::SortAsc()
{
	Sort(0, GetSize(), true);
}

// Sort ascending
void XScriptArray::SortAsc(asUINT startAt, asUINT count)
{
	Sort(startAt, count, true);
}

// Sort descending
void XScriptArray::SortDesc()
{
	Sort(0, GetSize(), false);
}

// Sort descending
void XScriptArray::SortDesc(asUINT startAt, asUINT count)
{
...
// internal
void XScriptArray::Sort(asUINT startAt, asUINT count, bool asc)
{
...
bool testSort(asIScriptFunction *func, int i, int j, asIScriptContext *ctx)
{
	ctx->Prepare(func);
	ctx->SetArgDWord(0, i);
	ctx->SetArgDWord(1, j);
...
void XScriptArray::StdSort(asIScriptFunction *func)
{
	int start = 0;
	int end = buffer->numElements;
	...
	// Insertion sort, not very optimized, but works
	for(int i = start + 1; i < end; i++)
	{
		Copy(tmp, GetArrayItemPointer(i));
		int j = i - 1;
		while(j >= start && testSort(func, j + 1, j, cmpContext))
		{
			Copy(GetArrayItemPointer(j + 1), GetArrayItemPointer(j));
			Copy(GetArrayItemPointer(j), tmp);
			j--;
		}
	}
...
// internal
void XScriptArray::CopyBuffer(SArrayBuffer *dst, SArrayBuffer *src)
{
	asIScriptEngine *engine = objType->GetEngine();
	if( subTypeId & asTYPEID_OBJHANDLE )
	{
...
// GC behaviour
void XScriptArray::EnumReferences(asIScriptEngine *engine)
{
...
// GC behaviour
void XScriptArray::ReleaseAllHandles(asIScriptEngine *)
{
	// Resizing to zero will release everything
	Resize(0);
}

void XScriptArray::AddRef() const
{
...
void XScriptArray::Release() const
{
	...
	if( asAtomicDec(refCount) == 0 )
	{
		...
		this->~XScriptArray();
		userFree(const_cast<XScriptArray*>(this));
	}
}

// GC behaviour
int XScriptArray::GetRefCount()
{
	return refCount;
}

// GC behaviour
void XScriptArray::SetFlag()
{
	gcFlag = true;
}

// GC behaviour
bool XScriptArray::GetFlag()
{
	return gcFlag;
}
...
static void ScriptArrayFactory_Generic(asIScriptGeneric *gen)
{
	...
	*(XScriptArray**)gen->GetAddressOfReturnLocation() = XScriptArray::Create(ot);
}
...
static void ScriptArrayFactory2_Generic(asIScriptGeneric *gen)
{
	...
	*(XScriptArray**)gen->GetAddressOfReturnLocation() = XScriptArray::Create(ot, length);
}
...
static void ScriptArrayListFactory_Generic(asIScriptGeneric *gen)
{
	...
	*(XScriptArray**)gen->GetAddressOfReturnLocation() = XScriptArray::Create(ot, buf);
}
...
static void ScriptArrayFactoryDefVal_Generic(asIScriptGeneric *gen)
{
	...
	*(XScriptArray**)gen->GetAddressOfReturnLocation() = XScriptArray::Create(ot, length, defVal);
}
...
{
	XScriptArray *other = (XScriptArray*)gen->GetArgObject(0);
	XScriptArray *self = (XScriptArray*)gen->GetObject();
	*self = *other;
